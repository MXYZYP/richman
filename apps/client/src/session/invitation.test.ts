...
describe('room invitations', () => {
  it('creates a canonical invitation URL with only the room query parameter', () => {
    ...
    const invitation = createInvitationUrl(source, '123456');

    expect(invitation.href).toBe('https://richman.example/game/table?room=123456');
    expect([...invitation.searchParams.entries()]).toEqual([['room', '123456']]);
  ...
  it('preserves the source origin and double-slash pathname', () => {
    ...
    expect(invitation.search).toBe('?room=123456');
    expect(invitation.hash).toBe('');
  });


  it('parses a six-digit room code after removing whitespace from an invitation URL', () => {
    expect(parseInvitationRoom(new URL('https://richman.example/game?room=12%203456'))).toBe('123456');
    expect(parseInvitationRoom(new URL('https://richman.example/game?room=%20%2012%203456%20%20'))).toBe('123456');
    expect(parseInvitationRoom(new URL('https://richman.example/game?room=1%09%202%0A3%0D4%205%206'))).toBe('123456');
    expect(parseInvitationRoom(new URL('https://richman.example/game?room=%20012345%20'))).toBe('012345');
    ...
    expect(parseInvitationRoom(new URL('https://richman.example/game?room=1234567'))).toBeNull();
  ...
  it('passes the exact canonical URL to the injected QR encoder without secrets', () => {
    ...
    const payload = createInvitationQrPayload(source, '567890', encode);

    const expectedUrl = createInvitationUrl(source, '567890');
  ...
  it('shares one canonical string between the copy affordance and the injected QR encoder', () => {
    ...
    const copyText = createInvitationUrlString(source, '000007');
    const payload = createInvitationQrPayload(source, '000007', encode);

    expect(copyText).toBe('https://richman.example/game/table?room=000007');
    expect(encode).toHaveBeenCalledOnce();
